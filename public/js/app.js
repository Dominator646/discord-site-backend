...
async function showBoard() {
    ...
    content.innerHTML = `
        ...
        <div class="board-container" id="boardWrapper">
            ...
            <div class="board-controls">
                <button class="trash-btn" onclick="clearMyLines()" title="Стереть мои линии">🗑</button>
            </div>
        </div>
    `;
    ...
    // Подгоняем размер холста
    resizeCanvas();
    window.addEventListener('resize', resizeCanvas);

    // Загружаем существующие линии
    const { data } = await supabase.from('board').select('*');
    allLines = data || [];
    redrawCanvas();

    // ПОДПИСКА НА REALTIME (Мгновенное обновление)
    supabase
        ...
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'board' }, payload => {
            allLines = allLines.filter(l => l.id !== payload.old.id);
            redrawCanvas();
        })
        .subscribe();
...
function redrawCanvas() {
    ...
    ctx.strokeStyle = '#ffd700'; // Жёлтый цвет
    ctx.lineWidth = 3;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
...
function draw(e) {
    ...
    // Рисуем временную линию локально для отзывчивости 1 в 1
...
function addPoint(e) {
    const x = (e.offsetX || (e.touches ? e.touches[0].clientX - canvas.offsetLeft : 0)) / canvas.width;
    const y = (e.offsetY || (e.touches ? e.touches[0].clientY - canvas.offsetTop : 0)) / canvas.height;
...
async function stopDrawing() {
    ...
    if (currentLine.length > 1) {
        // Отправляем линию в БД (Realtime сам раздаст её остальным)
        await supabase.from('board').insert({
            user_id: me.discord_id,
            points: JSON.stringify(currentLine)
        });
    }
}

async function clearMyLines() {
    if (!confirm('Стереть все твои линии на доске?')) return;
    await supabase.from('board').delete().eq('user_id', me.discord_id);
}
